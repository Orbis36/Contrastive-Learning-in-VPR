...
class NetVLAD(VPRModelTemplate):
    def __init__(self, model_cfg, cache_dataset):
        ...
        file_name = self.backbone.encoder_name.lower() + '_' + cache_dataset.split + \
            '_' + str(self.feature_selecter.num_clusters) + '_desc_cen.hdf5'
    ...
    def get_training_loss(self, batch_dict):
        loss_triplet = self.feature_selecter.get_feature_selected_loss(
            batch_dict)
        return loss_triplet

    def get_cluster(self, cache_dataset):

        # 在cluster时用db中的随机的500张照片来做聚类
        sample_selected = np.random.choice(len(cache_dataset.dbImages), self.nImg, replace=False)
        cluster_data_loader = DataLoader(dataset=ImagesFromList(cache_dataset.dbImages[sample_selected], transform=cache_dataset.augmentor),
                                         num_workers=cache_dataset.workers, batch_size=cache_dataset.bs_cluster, shuffle=False,
        ...
        with h5py.File(self.initcache_clusters_path, mode='w') as h5_file:
            with torch.no_grad():
                ...
                dbFeat = h5_file.create_dataset(
                    "descriptors", [self.cacheDescsNum, self.backbone.out_dim], dtype=np.float32)

                # tqdm(cluster_data_loader, desc='Iter'.rjust(15))
                for iteration, batch_dict in enumerate(cluster_data_loader, 1):
                    load_to_gpu(batch_dict=batch_dict)
                    batch_dict = self.forward(batch_dict)
                    feature_from_backbone = batch_dict['feature_map'].view(
                        cache_dataset.bs_cluster, self.backbone.out_dim, -1).permute(0, 2, 1)

                    # 这里netvlad是在layers中定义的L2-norm
                    image_descriptors = F.normalize(feature_from_backbone, p=2, dim=2)
                    batchix = (iteration - 1) * cache_dataset.bs_cluster * self.cacheDescPerImg
                    for ix in range(image_descriptors.size(0)):
                        ...
                        dbFeat[startix:startix + self.cacheDescPerImg,:] = \
            ...
            niter = 100
            kmeans = faiss.Kmeans(
                self.backbone.out_dim, self.feature_selecter.num_clusters, niter=niter, verbose=False, seed=123)
            kmeans.train(dbFeat[...])

            tqdm.write('====> Storing centroids ' +
                       str(kmeans.centroids.shape))
